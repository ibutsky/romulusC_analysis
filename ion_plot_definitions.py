...
def phase_median_frequency_profile(x, y, z, xmin = None, xmax = None, nbins = 50):
    if xmin == None:
        xmin = x.min()
    if xmax == None:
        xmax = x.max()
    ...
    for i in range(nbins):
        ...
        sample_2d = z[mask]
        for sample_array in sample_2d:
            sample = np.add(sample, sample_array)
        tot = np.sum(sample)
        cumsum = np.cumsum(sample)
    
        index_array = np.where(cumsum > 0.5 * tot)[0]
        if index_array.size != 0:
            index = index_array[0]
            median[i] = y[index]

    return centered_x_bins, median
...
def phase_median_profile(x_arr, y_arr, z_arr, res = 256, xmin = None, xmax = None, nbins = 60):
    if xmin == None:
        xmin = x_arr.min()
    if xmax == None:
        xmax = x_arr.max()
    xbins = np.linspace(xmin, xmax, nbins+1)
    # assumes x, y, z arrays are generated by yt ProfilePlot
    px, py = np.mgrid[x_arr.min():x_arr.max():res*1j, y_arr.min():y_arr.max():res*1j]
    x = px.ravel()
    y = py.ravel()
    z = z_arr.T.ravel()
    
    median = np.zeros(len(xbins))
    std = np.zeros(len(xbins))
    for i in np.arange(1, nbins):
        mask = (x < xbins[i]) & (x >= xbins[i-1])
        sample = y[mask] * z[mask] 
        median[i] = np.median(sample) / np.sum(z[mask])
        std[i] = np.std(sample)
    return xbins, median, std
